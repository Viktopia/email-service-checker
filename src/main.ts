...
async function runCheck(raw: string): Promise<void> {
    ...
    try {
        const [mxRecords] = await Promise.all([lookupMx(domain)]);
        const findings = identifyProviders(mxRecords);
        const consumerHit = consumerLookup(domain);
        ...
        window.gtag?.('event', 'check_email_service', {
            ...
            service: findings.length > 0
                ...
                : (consumerHit ? `consumer:${consumerHit.kind}` : 'Unknown'),
            mx_count: mxRecords.length,
        });
    } catch (err) {
        const msg = err instanceof LookupError
